...
- Operational framework: CP phases are generated by the same primitive collapse
  kernel that appears in the universal GKSL generator. In particular, for an
  appropriate basis of (effective) mode wavefunctions {ψ_i}:
      K_ij = Λ ∫ d^3x d^3y K(x,y) ψ_i*(x) ψ_j(y)
      δ_ij = arg(K_ij)

  In UDOF v4.0, we use the closed-form analytic overlap (when the kernel is in
  the validated bi-local family used throughout the codebase):
      K_ij = Λ * (4π/3) ℓ^3 [1 + (ℓ/σ_eff)^2]^(-3/2) * exp(i(φ0 + φ1 ℓ_eff))
  and the CP phase is δ = arg(K_ij).
"""
...
class CPViolationDerivation:
    ...
    Core hypothesis:
    1. CP violation is generated by the complex phase of the collapse-kernel
       overlap K_ij (no phenomenological CP insertion).
    2. Mixing angles come from TSI geometry; CP phases come from the kernel.
    3. Jarlskog invariants follow from the standard PDG parameterization once δ
       is fixed.

    Lock-file inputs (standards compliance):
    - cp_violation_parameters.phi0, phi1: kernel intrinsic phase parameters
    - cp_violation_parameters.ell_length: kernel length scale ℓ [m]
    - cp_violation_parameters.sigma_eff[_ckm|_pmns]: effective mode-width σ_eff [m]
    - cp_violation_parameters.ell_eff_ckm / ell_eff_pmns: effective overlap scale ℓ_eff [m]

    Back-compat:
    If phi0/phi1 are not present, the legacy calibrated mapping using
    cp_asymmetry_ckm/pmns is used as a fallback.
    """
    ...
    def __init__(self, locks: Dict[str, Any]):
        ...
        # Kernel overlap parameters for analytic CP phase generation
        # (preferred path; makes CP a downstream consequence of the primitive kernel)
        self.phi0 = cp_params.get('phi0', None)
        self.phi1 = cp_params.get('phi1', None)

        # Operational parameters (also used elsewhere in UDOF)
        self.Lambda_rate = float(cp_params.get('Lambda_rate', 1e23))  # s^-1
        self.ell_length = float(cp_params.get('ell_length', 2e-15))  # m

        # Effective overlap scales / widths (sector-specific if provided)
        self.sigma_eff_default = float(cp_params.get('sigma_eff', self.ell_length))
        self.sigma_eff_ckm = float(cp_params.get('sigma_eff_ckm', self.sigma_eff_default))
        self.sigma_eff_pmns = float(cp_params.get('sigma_eff_pmns', self.sigma_eff_default))
        self.ell_eff_ckm = float(cp_params.get('ell_eff_ckm', self.ell_length))
        self.ell_eff_pmns = float(cp_params.get('ell_eff_pmns', self.ell_length))

        # Legacy calibrated asymmetry parameters (fallback only)
        self.cp_asymmetry_ckm = float(cp_params.get('cp_asymmetry_ckm', 0.225))
        self.cp_asymmetry_pmns = float(cp_params.get('cp_asymmetry_pmns', 0.30))

    @staticmethod
    def _wrap_to_0_pi(phi: float) -> float:
        """
        Map a phase to the conventional [0, π] range.

        This is a convention choice for reporting δ; physical observables depend
        on invariant combinations.
        """
        two_pi = 2.0 * np.pi
        x = float(phi % two_pi)
        if x > np.pi:
    ...
    def kernel_overlap_Kij(
        self,
        sigma_eff: float,
        ell_eff: float
    ) -> complex:
        """
        Closed-form analytic collapse-kernel overlap K_ij.

        K_ij = Λ * (4π/3) ℓ^3 [1 + (ℓ/σ_eff)^2]^(-3/2) * exp(i(φ0 + φ1 ℓ_eff))
        """
        # If phi0/phi1 are not set, we cannot produce an analytic kernel phase.
        if self.phi0 is None or self.phi1 is None:
            return 0.0 + 0.0j

        ell = float(self.ell_length)
        sig = float(max(sigma_eff, 1e-60))
        prefactor = float(self.Lambda_rate) * (4.0 * np.pi / 3.0) * (ell ** 3)
        shape = (1.0 + (ell / sig) ** 2) ** (-1.5)
        phase = float(self.phi0) + float(self.phi1) * float(ell_eff)
        return complex(prefactor * shape * np.exp(1j * phase))

    def cp_phase_from_kernel(
        self,
        sector: str
    ) -> float:
        """
        Compute the sector CP phase δ from the kernel overlap phase arg(K_ij).
        """
        if sector.lower() == 'ckm':
            Kij = self.kernel_overlap_Kij(
                sigma_eff=self.sigma_eff_ckm, ell_eff=self.ell_eff_ckm
            )
        else:
            Kij = self.kernel_overlap_Kij(
                sigma_eff=self.sigma_eff_pmns, ell_eff=self.ell_eff_pmns
            )

        if Kij == 0.0j:
            return float('nan')
        return self._wrap_to_0_pi(float(np.angle(Kij)))
    ...
    def derive_ckm_phases_from_collapse(
        ...
        # CP phase from collapse kernel overlap (preferred).
        # Fallback: legacy calibrated mapping if analytic kernel params absent.
        delta_from_kernel = self.cp_phase_from_kernel('ckm')
        if np.isfinite(delta_from_kernel):
            delta_CP = delta_from_kernel
        else:
            delta_CP = 1.0 + self.cp_asymmetry_ckm * 2.0
            delta_CP = float(np.clip(delta_CP, 0.0, np.pi))
    ...
    def derive_pmns_phases_from_collapse(
        ...
        # CP phase from collapse kernel overlap (preferred).
        # Fallback: legacy calibrated mapping if analytic kernel params absent.
        delta_from_kernel = self.cp_phase_from_kernel('pmns')
        if np.isfinite(delta_from_kernel):
            delta_CP = delta_from_kernel
        else:
